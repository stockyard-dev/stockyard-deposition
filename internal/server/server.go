...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/auditentrys", s.list)
	s.mux.HandleFunc("POST /api/auditentrys", s.create)
	s.mux.HandleFunc("GET /api/auditentrys/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/auditentrys/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-deposition", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"auditentrys": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.AuditEntry{} }
	wj(w, map[string]any{"auditentrys": items, "count": len(items)})
}

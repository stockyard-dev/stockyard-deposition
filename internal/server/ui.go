...
<div class="hdr"><h1><span>Deposition</span></h1><div class="stats">Total: <b id="ct">-</b></div></div>
<div class="main">
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
<span style="font-size:.65rem;letter-spacing:2px;text-transform:uppercase;color:var(--rust)">All auditentrys</span>
<button class="btn btn-p" onclick="showCreate()">+ New</button>
</div>
<div id="list"></div>
</div>
<script>
async function load(){const r=await fetch('/api/auditentrys');const d=await r.json();document.getElementById('ct').textContent=d.count;
const el=document.getElementById('list');if(!d.auditentrys.length){el.innerHTML='<div class="empty">No auditentrys yet.</div>';return}
el.innerHTML=d.auditentrys.map(e=>'<div class="card"><div><div class="card-title">'+esc(e.name||e.title||e.id)+'</div><div class="card-sub">'+esc(e.created_at)+'</div></div><button class="btn btn-d" onclick="del(\''+e.id+'\')">Delete</button></div>').join('')}
function esc(s){return(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;')}
function showCreate(){const n=prompt('Name:');if(!n)return;fetch('/api/auditentrys',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:n})}).then(load)}
async function del(id){if(!confirm('Delete?'))return;await fetch('/api/auditentrys/'+id,{method:'DELETE'});load()}
